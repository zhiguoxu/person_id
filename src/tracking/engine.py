...
class TrackingEngine:
    ...
    def __init__(self, config: Config) -> None:
        ...
        Args:
            config: 系统配置，使用 config.tracking 子配置。
        """
        self._config = config
        self._tracking_cfg = config.tracking

        # Identity cache: track_id → PersonIdentity
        self._identity_cache: dict[int, PersonIdentity] = {}

        # Trail buffer: track_id → list[(cx, cy)]
        self._trails: dict[int, list[tuple[float, float]]] = {}

        # Detection cache for area-trend computation (attention engine may use)
    ...
    def update(
        ...
        for row in results:
            ...
            # Update area history
            area = (x2 - x1) * (y2 - y1)
            area_hist = self._area_history.setdefault(track_id, [])
            area_hist.append(area)
            if len(area_hist) > self._MAX_TRAIL_LEN:
                area_hist[:] = area_hist[-self._MAX_TRAIL_LEN:]

            # Lookup identity cache
            ...
            person = TrackedPerson(
                ...
                identity_status=(
                    identity.status if identity
                    else IdentityStatus.IDENTIFYING
                ),
    ...
    def inject_identity(
        ...
        Args:
            ...
            status: 身份状态。
        """
        self._identity_cache[track_id] = PersonIdentity(
            person_id=person_id,
        ...
        logger.debug(
            "Identity injected: track_id={} → {} ({}, conf={:.2f})",
            track_id, display_name, status.value, confidence,
        )
    ...
    def get_identity(self, track_id: int) -> Optional[dict]:
        ...
        Returns:
            身份字典 {person_id, display_name, confidence, status}，
            若不存在返回 None。
        """
        identity = self._identity_cache.get(track_id)
        if identity is None:
    ...
    def remove_track(self, track_id: int) -> None:
        ...
        self._area_history.pop(track_id, None)
    ...
    def get_area_history(self, track_id: int) -> list[float]:
        """
        获取追踪 ID 的 bbox 面积历史。
        ...
        Returns:
            面积历史列表（最近 MAX_TRAIL_LEN 帧）。
        """
        return list(self._area_history.get(track_id, []))
    ...
    def _create_tracker(self) -> object:
        ...
        if _BOXMOT_AVAILABLE:
            try:
                ...
                device = torch.device(self._config.detection.yolo_device)
                tracker = BoTSORT(
                    reid_weights=Path(""),
                    device=device,
                    half=False,
                    track_high_thresh=self._tracking_cfg.track_high_thresh,
                    track_low_thresh=self._tracking_cfg.track_low_thresh,
                    new_track_thresh=self._tracking_cfg.new_track_thresh,
                    track_buffer=self._tracking_cfg.track_buffer,
                    match_thresh=self._tracking_cfg.match_thresh,
                    cmc_method=self._tracking_cfg.cmc_method,
                    with_reid=False,
                )
                ...
                import traceback
                logger.warning(
                    "Failed to create BoT-SORT tracker, falling back to SimpleTracker:\n{}",
                    traceback.format_exc()
                )
                return _SimpleTracker(max_age=self._tracking_cfg.track_buffer)
        else:
            logger.warning(
                "boxmot not installed — using SimpleTracker fallback. "
                "Install boxmot for production tracking: pip install boxmot"
            )
            return _SimpleTracker(max_age=self._tracking_cfg.track_buffer)
    ...
    def _cleanup_stale(self, active_ids: set[int]) -> None:
        """Remove trail / area history entries for tracks no longer active."""
        stale_trail = [tid for tid in self._trails if tid not in active_ids]
        for tid in stale_trail:
            # Keep identity cache — it may be needed for spatial memory recovery
            self._trails.pop(tid, None)
            self._area_history.pop(tid, None)
