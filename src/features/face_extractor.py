...
class FaceExtractor:
    ...
    def __init__(self, config: FaceConfig) -> None:
        ...
        Args:
            config: 人脸配置, 包含模型名称、设备 ID、检测尺寸等。
        """
        self.config = config
        self._app = None
    ...
    def extract(
        ...
        px2 = min(w, int(person_bbox[2]))
        py2 = min(h, int(person_bbox[3]))
        person_height = py2 - py1

        # Focus on upper body for face detection
        crop_y2 = min(py2, py1 + int(person_height * 0.6))
        ...
        best_face = max(faces, key=lambda f: _face_area(f))
        ...
        try:
            ...
            face_bbox = best_face.bbox.astype(np.float32)
            face_bbox[0] += px1
            face_bbox[1] += py1
            face_bbox[2] += px1
            face_bbox[3] += py1
            ...
            landmarks = best_face.kps.astype(np.float32)  # (5, 2)
            landmarks[:, 0] += px1
            landmarks[:, 1] += py1
            ...
            quality = self._compute_quality(face_bbox, det_score, crop.shape)
    ...
    def _compute_quality(
        ...
        face_size = min(float(face_w), float(face_h))

        # Size score: penalize very small faces
        size_score = min(1.0, face_size / self.config.min_face_size)
